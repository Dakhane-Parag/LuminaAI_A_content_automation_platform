...
export function AppSidebar() {
  ...
  return (
    ...
    >
      {/* ── Brand Logo ──────────────────────────────────────────────────── */}
      <div
        className="flex items-center gap-3 px-4 py-5 border-b cursor-pointer transition-colors hover:bg-white/5"
      ...
      <nav className="flex-1 py-4 space-y-1 px-2">
        {NAV_ITEMS.map((item) => (
          <NavLink
            ...
            to={item.to}
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-150 group/item overflow-hidden whitespace-nowrap ${isActive
                ? 'text-white'
      ...
      <div className="px-2 py-3 border-t flex items-center gap-3 overflow-hidden whitespace-nowrap" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
        ...
        <div className="overflow-hidden transition-opacity duration-200 opacity-0 group-hover:opacity-100">
          ...
          <p className="text-[10px] text-[#00A86B] tracking-tight flex items-center gap-1">
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4">
        {/* Settings */}
        <button onClick={() => navigate('/settings')} className="p-2 rounded-full text-white/50 hover:text-white hover:bg-white/5 transition-colors">
          <SettingsIcon />
        </button>
        {/* Divider */}
